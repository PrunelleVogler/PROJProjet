...
using Libdl
timeLimit = 100
instanceName = "Data/processed/40_USA-road-d.BAY.gr"
...
function bac_algorithm(instanceName, timeLimit)
    time_begin = time()
    ...
    elseif primal_status(m) == MOI.FEASIBLE_POINT
        upper_bound = 0
        lower_bound = objective_bound(m)
    end
...
bac_algorithm(instanceName, timeLimit)